...
namespace DryIoc.Wcf {
    public class DryIocServiceHostFactory : ServiceHostFactory {
        ...
        protected override ServiceHost CreateServiceHost(Type contractType, Uri[] baseAddresses) {
            ...
            var registrationInfo = s_container.GetServiceRegistrations().Single(x => x.ServiceType == contractType);
            ...
            if (HasInstanceContextModeSingle(implementationType) && AssertIsRegisteredAsSingleton(registrationInfo)) {
                return new DryIocServiceHost(s_container, s_container.Resolve(contractType), baseAddresses);
            }
            return new DryIocServiceHost(s_container, contractType, implementationType, baseAddresses);
        }
